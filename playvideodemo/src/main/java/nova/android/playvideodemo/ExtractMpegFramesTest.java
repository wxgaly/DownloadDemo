...
public class ExtractMpegFramesTest {
    ...
    private static final File FILES_DIR = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + File
            .separator + "pic");
    private static final String INPUT_FILE = "test.mp4";
    private static final int MAX_FRAMES = 10;       // stop extracting after this many
    private static final int TIME_STEP = 29;
    ...
    public void testExtractMpegFrames() throws Throwable {
        new ExtractMpegFramesWrapper(this).runTest();
    }

    public void setiRender(IRender render) {
        this.mRender = render;
    }
    ...
    private class ExtractMpegFramesWrapper implements Runnable {
        ...
        public ExtractMpegFramesWrapper(ExtractMpegFramesTest test) {
            mTest = test;
        }
        ...
        public void runTest() throws Throwable {
            Thread th = new Thread(this, "codec test");
            th.start();
            th.join();
    ...
    void doExtract(MediaExtractor extractor, int trackIndex, MediaCodec decoder,
        ...
        Log.e(TAG, "extractor.getTrackCount():" + extractor.getTrackCount());
        int inputChunk = 0;
        int decodeCount = 0;
...
//                        if (decodeCount < MAX_FRAMES) {
                        ...
                        outputSurface.drawFrame(outputFile.toString());
                        long drawTime = System.currentTimeMillis() - startWhen;
                        frameSaveTime += drawTime;
//                        }
                        decodeCount++;

                        try {
//                            Log.d(TAG, "drawTime: " + drawTime);
                            if (drawTime < TIME_STEP) {
                                Thread.sleep(TIME_STEP - drawTime);
                            }
        ...
        Log.d(TAG, "Saving " + numSaved + " frames took " +
                (frameSaveTime / numSaved) + " us per frame");
    }
    ...
    private class CodecOutputSurface implements SurfaceTexture.OnFrameAvailableListener {
        ...
        private ByteBuffer mPixelBuf;                       // used by drawFrame()
         ...
         */
        public void drawFrame(String filename) throws IOException {
